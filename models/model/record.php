 ...
 */
class PMLI_Model_Record extends PMLI_Model {
	/**
	 * Initialize model
	...
	/**
	 * @see PMLI_Model::getBy()
	 * @return PMLI_Model_Record
	 */
	 ...
	 * @return PMLI_Model_List
	 */
	public function getRelated($model, $keyAssoc = NULL) {
		$related = new $model();
		if ( ! empty($this->id)) {
			if (is_null($keyAssoc)) {
				$defaultPrefix = strtolower(preg_replace('%^' . strtoupper(PMLI_Plugin::PREFIX) . '|_Record$%', '', get_class($this)));
				$keyAssoc = array();
		...
		return $related instanceof PMLI_Model_List ? $related->convertRecords() : $related;
	}
